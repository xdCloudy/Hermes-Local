...
const HERMES_LOCAL_MODE = configureHermesLocalDesktopEnvironment()
...
// The Hermes Local client is root-owned. Development resolves the separately
// reconstructed Hermes Agent checkout through this explicit harness boundary.
const SOURCE_REPO_ROOT = path.resolve(APP_ROOT, '../../source/hermes-agent')
...
const hermesLocalDashboardView = createHermesLocalDashboardViewController({
  emitState: state => {
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('hermes:local:dashboard:state', state)
    }
  },
...
// Capture permissions cross the renderer/main-process trust boundary. Only an
// owned, trusted Hermes renderer may request audio capture; video and unrelated
// permissions remain denied by default.
function rendererPermissionRequest(webContents, permission, details: any = {}) {
  return {
    details,
    hasWindowOwner: Boolean(webContents && BrowserWindow.fromWebContents(webContents)),
    permission,
    url: webContents?.getURL?.() || details?.requestingUrl || ''
  }
}
...
function installMediaPermissions() {
  const locations = {
    ...
    packagedRendererUrl: pathToFileURL(resolveRendererIndex()).toString()
  }

  session.defaultSession.setPermissionRequestHandler((webContents, permission, callback, details) => {
    callback(shouldAllowRendererPermission(rendererPermissionRequest(webContents, permission, details), locations))
  })

  session.defaultSession.setPermissionCheckHandler((webContents, permission, _origin, details) => {
    return shouldAllowRendererPermission(rendererPermissionRequest(webContents, permission, details), locations)
  })
}
...
async function ensureBackend(profile) {
  const key = normalizeBackendProfile(profile, primaryProfileKey())
...
async function startHermes() {
  ...
  const connectionPromise = (async () => {
    ...
    if (HERMES_LOCAL_MODE) {
      await advanceBootProgress('backend.workstation', 'Starting Hermes Local workstation', 16)
      await ensureHermesLocalWorkstationReady()
    }
...
function createWindow() {
  ...
  const keepWindowHiddenForAcceptance = process.env.HERMES_DESKTOP_TEST_HIDDEN === '1'
  ...
  mainWindow.once('ready-to-show', () => {
    if (mainWindow && !mainWindow.isDestroyed() && !keepWindowHiddenForAcceptance) {
      mainWindow.show()
    }
  ...
  if (process.env.TEST_WORKER_INDEX !== undefined && !keepWindowHiddenForAcceptance) {
...
registerHermesLocalControlIpc(hermesLocalDashboardView)
...
ipcMain.handle('hermes:terminal:start', async (event, payload = {}) => {
  ...
  const requestedMode = String(payload?.mode || 'shell')

  if (requestedMode !== 'shell' && requestedMode !== 'hermes-tui') {
    throw new Error('Unsupported terminal mode')
  }

  const cwd = safeTerminalCwd(payload?.cwd)

  const localLaunch =
    requestedMode === 'hermes-tui'
      ? hermesLocalTuiLaunch()
      : { ...terminalShellCommand(), cwd, env: {} }

  const { args, command, name } = localLaunch
  ...
  if (remote && requestedMode === 'hermes-tui') {
    throw new Error('Hermes Local TUI is available only on the local workstation')
  }
  ...
  const ptyProcess = remote
    ...
    : nodePty.spawn(command, args, {
        cols,
        cwd: localLaunch.cwd,
        env: { ...terminalShellEnv(), ...localLaunch.env },
        name: 'xterm-256color',
        rows
      })
  ...
  return { cwd: remote ? null : cwd, id, pid: ptyProcess.pid, shell: remote ? 'ssh' : name }
})
...
ipcMain.handle('hermes:updates:check', async () => {
  if (HERMES_LOCAL_MODE) {
    return checkHermesLocalDesktopUpdates()
  }
...
ipcMain.handle('hermes:updates:apply', async (_event, payload) => {
  if (HERMES_LOCAL_MODE) {
    const result = await applyHermesLocalDesktopUpdate(payload || {})

    if (result.handedOff) {
      emitUpdateProgress({
        stage: 'restart',
        message: 'Closing Hermes Local so the trusted updater can replace and validate the launcher.',
        percent: 90
      })
...
app.on('before-quit', event => {
  ...
  // The native embedded dashboard is owned by the primary window and must not
  // keep its isolated renderer/session alive during application teardown.
  hermesLocalDashboardView.dispose()
