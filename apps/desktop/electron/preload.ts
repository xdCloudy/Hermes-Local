...
contextBridge.exposeInMainWorld('hermesDesktop', {
  ...
  localWorkstation: {
    snapshot: () => ipcRenderer.invoke('hermes:local:snapshot'),
    loginItem: {
      get: () => ipcRenderer.invoke('hermes:local:login-item:get'),
      set: enabled => ipcRenderer.invoke('hermes:local:login-item:set', enabled)
    },
    startAction: (action, input) => ipcRenderer.invoke('hermes:local:action:start', action, input),
    actionStatus: taskId => ipcRenderer.invoke('hermes:local:action:status', taskId),
    listActions: () => ipcRenderer.invoke('hermes:local:action:list'),
    cancelAction: taskId => ipcRenderer.invoke('hermes:local:action:cancel', taskId),
    pauseAction: taskId => ipcRenderer.invoke('hermes:local:action:pause', taskId),
    resumeAction: taskId => ipcRenderer.invoke('hermes:local:action:resume', taskId),
    retryAction: taskId => ipcRenderer.invoke('hermes:local:action:retry', taskId),
    openActionResult: taskId => ipcRenderer.invoke('hermes:local:action:open-result', taskId),
    logs: (name, lines) => ipcRenderer.invoke('hermes:local:logs', name, lines),
    saveProfile: (profile, originalName) => ipcRenderer.invoke('hermes:local:profile:save', profile, originalName),
    deleteProfile: name => ipcRenderer.invoke('hermes:local:profile:delete', name),
    selectProfile: name => ipcRenderer.invoke('hermes:local:profile:select', name),
    registerModel: model => ipcRenderer.invoke('hermes:local:model:register', model),
    removeModel: id => ipcRenderer.invoke('hermes:local:model:remove', id),
    selectModel: id => ipcRenderer.invoke('hermes:local:model:select', id),
    saveSettings: settings => ipcRenderer.invoke('hermes:local:settings:save', settings),
    dashboard: {
      state: () => ipcRenderer.invoke('hermes:local:dashboard:state'),
      show: bounds => ipcRenderer.invoke('hermes:local:dashboard:show', bounds),
      resize: bounds => ipcRenderer.invoke('hermes:local:dashboard:resize', bounds),
      hide: () => ipcRenderer.invoke('hermes:local:dashboard:hide'),
      reload: () => ipcRenderer.invoke('hermes:local:dashboard:reload'),
      onState: callback => {
        const listener = (_event, state) => callback(state)
        ipcRenderer.on('hermes:local:dashboard:state', listener)

        return () => ipcRenderer.removeListener('hermes:local:dashboard:state', listener)
      }
    },
    openDashboard: () => ipcRenderer.invoke('hermes:local:dashboard:open'),
    openPath: relativePath => ipcRenderer.invoke('hermes:local:path:open', relativePath),
    openRoot: () => ipcRenderer.invoke('hermes:local:root:open'),
    trustSnapshot: () => ipcRenderer.invoke('hermes:local:trust:snapshot'),
    setTrustPolicy: input => ipcRenderer.invoke('hermes:local:trust:policy', input),
    exportTrustDiagnostics: () => ipcRenderer.invoke('hermes:local:trust:diagnostics')
  },
