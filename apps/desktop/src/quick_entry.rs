...
pub fn load_settings(path: &Path) -> Result<QuickEntrySettings, String> {
    ...
    let bytes =
        fs::read(path).map_err(|error| format!("Could not read Quick Entry settings: {error}"))?;
...
mod tests {
    ...
    fn parser_matches_electron_alias_order_duplicate_and_key_rules() {
        ...
        assert_eq!(
            parse_shortcut("Alt+f5"),
            ShortcutParse::Valid("Alt+F5".to_owned())
        );
        assert_eq!(
            parse_shortcut("Meta+/"),
            ShortcutParse::Valid("Super+/".to_owned())
        );
        assert_eq!(
        ...
        assert_eq!(
            parse_shortcut("K"),
            ShortcutParse::Invalid(ShortcutError::NoModifier)
        );
        assert_eq!(
            parse_shortcut("Shift+Control"),
            ShortcutParse::Invalid(ShortcutError::NoKey)
        );
        assert_eq!(
            parse_shortcut("Shift+A+B"),
            ShortcutParse::Invalid(ShortcutError::InvalidKey)
        );
        assert_eq!(
            parse_shortcut("A+Shift"),
            ShortcutParse::Invalid(ShortcutError::InvalidModifier)
        );
        assert_eq!(
            parse_shortcut("Ctrl+Escape"),
            ShortcutParse::Invalid(ShortcutError::Reserved)
        );
    }
