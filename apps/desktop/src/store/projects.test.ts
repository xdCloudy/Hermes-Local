...
describe('resolveNewSessionCwd', () => {
  ...
  it('falls back to the configured default outside Home', () => {
    expect(resolveNewSessionCwd()).toBe('/home/user/configured')
  })

  it('does not inherit a persisted project scope for a new chat', () => {
    $projectTree.set([
      {
        id: 'p_previous',
...
describe('stable project registry', () => {
  const project = {
    archived: false,
    board_slug: null,
    color: null,
  ...
  it('recovers a moved path without changing the project id', async () => {
    const repaired = {
      ...project,
      folders: [{ ...project.folders[0], path: '/work/new', path_key: '/work/new', path_state: 'moved' as const }],
      path_state: 'moved' as const,
      path_aliases: [
        { path: '/work/old', path_key: '/work/old', repository_id: 'github.com/example/demo', replaced_at: 2 }
      ],
      primary_path: '/work/new'
    }
    const request = vi.fn(async (method: string) => {
      if (method === 'projects.recover_path') {
        return { project: repaired }
      }

      return method === 'projects.list'
        ? { active_id: null, projects: [repaired] }
        : { active_id: null, projects: [], scoped_session_ids: [] }
    })

    activeGateway.mockReturnValue({ connectionState: 'open', request } as never)

    const result = await recoverProjectPath('p_stable', '/work/old', '/work/new')

    expect(result.id).toBe('p_stable')
    expect($projects.get()[0]?.id).toBe('p_stable')
    expect($projects.get()[0]?.primary_path).toBe('/work/new')
    expect($projectTree.get()[0]?.path).toBe('/work/new')
    expect(request).toHaveBeenCalledWith(
      'projects.recover_path',
      expect.objectContaining({ id: 'p_stable', new_path: '/work/new', old_path: '/work/old' })
    )
  })

  it('refreshes persisted accessibility state from the registry', async () => {
    const refreshed = { ...project, path_state: 'inaccessible' as const }
    const request = vi.fn().mockResolvedValue({ projects: [refreshed] })
    activeGateway.mockReturnValue({ connectionState: 'open', request } as never)

    await refreshProjectRegistry('p_stable')

    expect(request).toHaveBeenCalledWith('projects.registry_refresh', { id: 'p_stable' })
    expect($projects.get()[0]?.path_state).toBe('inaccessible')
  })
})
