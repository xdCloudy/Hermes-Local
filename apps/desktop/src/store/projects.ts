...
// The cwd a bare NEW chat should start in. Project/worktree "+" actions pass an
// explicit workspace target through startSessionInWorkspace; the sidebar's
// persisted view scope is not session authority. Letting it choose the cwd made
// renderer reloads and reconnects attach the last-viewed (sometimes install)
// repository to an unrelated new chat.
...
export async function addProjectFolder(
  id: string,
  path: string,
  opts: { label?: string; isPrimary?: boolean; repositoryId?: string } = {}
): Promise<void> {
  const snap = snapshotProjects()
  ...
  if (trimmed) {
    const folder = {
      ...
      added_at: 0,
      path_key: null,
      repository_id: opts.repositoryId ?? null,
      path_state: 'missing' as const,
  ...
  await persistOrRollback(snap, () =>
    gatewayRequest('projects.add_folder', {
      id,
      path,
      label: opts.label,
      is_primary: opts.isPrimary ?? false,
...
export async function refreshProjectRegistry(id?: string): Promise<ProjectInfo[]> {
  const res = await gatewayRequest<{ projects: ProjectInfo[] }>('projects.registry_refresh', { id })

  if (id && res.projects[0]) {
    const refreshed = res.projects[0]
    $projects.set($projects.get().map(project => (project.id === id ? refreshed : project)))
    $projectTree.set(
      $projectTree.get().map(project =>
        project.id === id ? { ...project, path: refreshed.primary_path ?? refreshed.folders[0]?.path ?? null } : project
      )
    )
  } else if (!id) {
...
export async function recoverProjectPath(
  ...
  if (trimmed) {
    $projects.set(
      snap.projects.map(project =>
        project.id === id
          ? {
              ...project,
              primary_path: project.primary_path === oldPath ? trimmed : project.primary_path,
              path_state: 'moved',
              folders: project.folders.map(folder =>
                folder.path === oldPath
                  ? {
                      ...
                      path: trimmed,
                      path_key: null,
                      repository_id: repositoryId ?? folder.repository_id,
                      path_state: 'moved'
                    }
                  : folder
              )
    ...
    $projectTree.set(snap.tree.map(project => (project.id === id && project.path === oldPath ? { ...project, path: trimmed } : project)))
  }

  let repaired: ProjectInfo | null = null
  await persistOrRollback(snap, async () => {
    const response = await gatewayRequest<{ project: ProjectInfo }>('projects.recover_path', {
      id,
      old_path: oldPath,
      new_path: newPath,
      repository_id: repositoryId
    })
    repaired = response.project
  })

  if (!repaired) {
    throw new Error('Project path recovery returned no project')
  }

  $projects.set($projects.get().map(project => (project.id === id ? repaired! : project)))
  reconcileProjects()
