...
const HERMES_LOCAL_ROOT = path.resolve(DESKTOP_ROOT, '..', '..')
const HERMES_AGENT_ROOT = path.join(HERMES_LOCAL_ROOT, 'source', 'hermes-agent')
 ...
 *    (we want the dev checkout at HERMES_AGENT_ROOT)
 *  - HERMES_DESKTOP_HERMES_ROOT → HERMES_AGENT_ROOT (dev checkout resolution)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...
    HERMES_DESKTOP_HERMES_ROOT: HERMES_AGENT_ROOT,
...
export function findElectron(): string {
  ...
  const localElectron = path.join(HERMES_LOCAL_ROOT, 'node_modules', 'electron', 'dist', 'electron')
