...
test('composeInstallStamp keeps bootstrap pinned to Agent while recording product provenance', () => {
  const agentCommit = '9'.repeat(40)
  const harnessTree = '8'.repeat(40)
  const productCommit = '7'.repeat(40)
  const payload = composeInstallStamp(
    {
      product: { client: { sourcePath: 'apps/desktop' } },
      sources: {
        hermesAgent: {
          commit: agentCommit,
          branch: 'main',
    ...
    { commit: productCommit, branch: 'feature/native-client', dirty: true, source: 'local' },
    '2026-08-10T12:00:00.000Z'
  )

  assert.equal(payload.commit, agentCommit)
  assert.equal(payload.productCommit, productCommit)
  assert.equal(payload.harnessTree, harnessTree)
  assert.equal(payload.clientSource, 'apps/desktop')
