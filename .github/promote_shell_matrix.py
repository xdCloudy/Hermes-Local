...
if len(capability_rows) != 127:
    raise SystemExit(f"expected 127 capability rows, found {len(capability_rows)}")
...
actual = {stage: 0 for stage in expected}
for row in capability_rows:
    for stage in actual:
        ...
        raise SystemExit(f"unclassified capability row: {row}")
if actual != expected:
    raise SystemExit(f"stage counts mismatch: {actual!r}")

shell_rows = [row for row in capability_rows if row.startswith("| SH-")]
if len(shell_rows) != 15 or any("| A4 Auto-verified |" not in row for row in shell_rows):
    raise SystemExit("not every Shell row is A4 after promotion")
