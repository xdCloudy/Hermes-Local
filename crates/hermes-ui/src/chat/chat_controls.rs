...
pub(super) fn ChatControls(session: Option<String>) -> Element {
    ...
    let directive = |command: String, services: AppServices, stored: String, mut status: Signal<String>, mut busy: Signal<bool>| {
        spawn(async move {
            let result = match session_runtime(&services, &stored).await {
                Ok(runtime) => services.sessions.execute_directive(&runtime, &command).await,
                Err(error) => Err(hermes_core::ServiceError::Platform(error)),
            };
        ...
        div { class: "chat-runtime-controls", aria_label: "Chat runtime controls",
            ...
            if let Some(stored) = session.clone() {
                select { disabled: busy(), aria_label: "Approval mode", onchange: {
                    let services = services.clone(); let stored = stored.clone();
                    move |event: Event<FormData>| { busy.set(true); directive(format!("/approvals {}", event.value()), services.clone(), stored.clone(), status, busy); }
                },
                    option { value: "manual", "Approvals: manual" }
                    option { value: "smart", "Approvals: smart" }
                    option { value: "off", "Approvals: off" }
                }
                input { value: "{tools}", aria_label: "Tool control", placeholder: "tools: list | +name | -name", oninput: move |event| tools.set(event.value()) }
                button { disabled: busy(), onclick: {
                    let services = services.clone(); let stored = stored.clone();
                    move |_| { let value = tools().trim().to_owned(); if !value.is_empty() && value.len() <= 256 { busy.set(true); directive(format!("/tools {value}"), services.clone(), stored.clone(), status, busy); } }
                }, "Apply tools" }
                button { disabled: busy(), onclick: {
                    let services = services.clone(); let stored = stored.clone();
                    move |_| { busy.set(true); directive("/approvals off".into(), services.clone(), stored.clone(), status, busy); }
                }, "YOLO" }
            }
        }
        if !status().is_empty() { small { role: "status", "{status}" } }
    }
}
