...
Assert-Contains -Text $benchmark -Expected "-LifecycleState 'restoration-pending'" -Message 'Native benchmark results are not checkpointed before restoration.'
Assert-Contains -Text $benchmark -Expected 'Restore-HermesBenchmarkStack' -Message 'Benchmark lacks replacement-supervisor recovery.'
Assert-Contains -Text $benchmark -Expected '-AllowControllerReplacement' -Message 'Benchmark cannot accept a healthy replacement supervisor.'
...
Assert-Contains -Text $gatewaySnapshot -Expected 'runtimeTimestampAdvisory' -Message 'Gateway snapshot still treats a state-change timestamp as a heartbeat.'
Assert-Contains -Text $gatewaySnapshot -Expected 'authoritative-process-and-platform-state' -Message 'Gateway health basis is not explicit.'
if ($gatewaySnapshot.Contains('healthy = otherwise_healthy and (not runtime_stale', [System.StringComparison]::Ordinal)) {
    throw 'Gateway health still fails solely because the runtime state-change timestamp is old.'
}
